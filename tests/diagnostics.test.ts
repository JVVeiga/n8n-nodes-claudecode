...
describe('buildDiagnostics — optional agent fields', () => {
	const base = () => ({
		messages: streams.success(),
		params: paramsFor(),
		permissionMode: 'bypassPermissions',
	...
	it('without extra, none of the new keys exists and the shape is unchanged', () => {
		const plain = buildDiagnostics(base());
		for (const key of NEW_KEYS) assert.equal(key in plain, false, `${key} must be absent`);
		assert.deepEqual(buildDiagnostics({ ...base(), extra: {} }), plain);
		assert.deepEqual(
			buildDiagnostics({
				...base(),
				extra: {
	...
	it('each field appears only when given', () => {
		...
		const plainKeys = Object.keys(buildDiagnostics(base()));
		for (const [key, value] of Object.entries(given)) {
			const d = buildDiagnostics({ ...base(), extra: { [key]: value } });
